...
class TestCommonTimesheet(TransactionCase):
    ...
    def setUp(self):
        ...
        rule = self.env.ref('timesheet_grid.rocker_timesheet_rule_approver_update', raise_if_not_found=False)
        if rule:
            rule.active = False
        ...
        self.user_employee = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.ref('rocker_timesheet.group_rocker_timesheet_user')])],
        })
        ...
        self.user_manager = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.ref('rocker_timesheet.group_timesheet_manager')])],
        })
        # employees
